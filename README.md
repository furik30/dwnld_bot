...
```
curl -sSL https://raw.githubusercontent.com/furik30/dwnld_bot/install.sh | bash
```
...
3. Конфигурация
    -   Откройте файл `.env` через `downloader edit`
...
1. Положите файлы `youtube_cookies.txt`, `instagram_cookies.txt` и `tiktok_cookies.txt` в формате Netscape (см [docs/README_Cookies.md]) в папку /downloader/bot/cookies/.
